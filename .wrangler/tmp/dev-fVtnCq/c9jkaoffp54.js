// .wrangler/tmp/bundle-g8L0L6/checked-fetch.js
...
// .wrangler/tmp/pages-s5BNic/bundledWorker-0.37665165883438134.mjs
...
async function C(e, { request: t, assetsFetcher: r, ctx: s }, { path: a, searchParams: i }) {
  ...
  try {
    switch (e?.type) {
      ...
      case "override": {
        n = S(await r.fetch(A(c, e.path ?? a))), e.headers && R(n.headers, e.headers);
        break;
      }
      case "static": {
...
async function G(e) {
  ...
  try {
    ...
    switch (e.method) {
      case "GET": {
        let i = z(e, fe), n = await s.get(a, { softTags: i });
      ...
      case "POST": {
        let i = await e.json();
        return i.data.tags === void 0 && (i.tags ??= z(e, pe) ?? []), await s.set(a, i), new Response(null, { status: 200 });
      }
      default:
...
function ge() {
  ...
  globalThis.fetch = async (...t) => {
    ...
    return s || (s = await G(r), s) ? s : (Re(r), e(r));
  };
}
async function ye(e) {
...
var k = class {
  ...
  applyLocaleRedirects(t) {
    if (!t.locale?.redirect || !/^\^(.)*$/.test(t.src) && t.src !== this.path || this.headers.normal.has("location"))
      return;
    let { locale: { redirect: s, cookie: a } } = t, i = a && this.cookies[a], n = I(i ?? ""), o = I(this.reqCtx.request.headers.get("accept-language") ?? ""), p = [...n, ...o].map((y) => s[y]).filter(Boolean)[0];
    if (p) {
...
// .wrangler/tmp/pages-s5BNic/c9jkaoffp54.js
...
// .wrangler/tmp/bundle-g8L0L6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-g8L0L6/middleware-loader.entry.ts
